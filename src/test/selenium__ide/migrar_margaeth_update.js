describe('Vamos ver...', function() {
	//it('load page', function(){
		...
		var clientes = [
["642","ZILDETE MARIA","(31) 7186-6754","",""],
		...
		clientes.forEach(function(cliente){
			...
			browser.driver.sleep(1000);
			browser.driver.findElement(by.xpath('//*[@id="main"]/div[2]/span/form/div/div/div/span/div/div/div/input')).sendKeys(cliente[1].replace("(",' ').replace(')', '').replace('*', ' funcionario').replace('.','').replace('-','').replace('/','').replace("(",' ').replace(')', '').replace('*', ' funcionario').replace('.','').replace('-','').replace('/','') );
			browser.driver.sleep(1000);
