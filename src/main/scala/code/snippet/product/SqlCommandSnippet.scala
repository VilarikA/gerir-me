...
//			super.page
	...
	def list(xhtml: NodeSeq): NodeSeq = {
			...
			bind("f", xhtml,"name" -> Text(cc.name.is),
							"obs" -> Text(cc.obs.is),
							"actions" -> <a class="btn" href={"/company_log/sql_command?id="+cc.id.is}>Editar</a><a class="btn success" href={"/company_log/sql_run?id="+cc.id.is}>Executar</a>,
	...
	def maintain = {
		try{
		    ...
		    "name=obs" #> (SHtml.text(cc.obs.is, cc.obs(_))++SHtml.hidden(process))
		}catch {
		    case e: NoSuchElementException => S.error("Sql não existe!")
