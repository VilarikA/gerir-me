...
trait EbProtoExtendedSession[T <: EbProtoExtendedSession[T]] extends
KeyedMapper[Long, T] {
  self: T =>
  ...
  object unit extends MappedLong(this)
  object company extends MappedLong(this)
...
trait MetaEbProtoExtendedSession[T <: EbProtoExtendedSession[T]] extends
KeyedMetaMapper[Long, T] with net.liftweb.common.Logger {
  self: T =>
  ...
  def logUserIdIn(uid: String, company:Long, unit:Long): Unit

  def recoverUserId: Box[String]

  def userDidLogin(uid: UserType) {
    userDidLogout(Full(uid))
    val userId = uid.userIdAsString
    val inst = create
    inst.userId(userId).unit(uid.unit).company(uid.company).save
      ...
      (recoverUserId, S.findCookie(CookieName)) match {
        case (Empty, Full(c)) =>
          find(By(cookieId, c.value openOr "")) match {
            ...
            case Full(es) => logUserIdIn(es.userId.get, es.company.get, es.unit.get)
            case _ =>
          }
